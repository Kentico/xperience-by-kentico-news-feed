...
import { ProductNewsFeedCard, ProductNewsFeedItem } from '../components/ProductNewsFeedCard';

interface ProductNewsFeedLayoutProps {
  feedTitle: string;
  feedShortDescription: string;
  feedItems: ProductNewsFeedItem[];
...
type RefreshFeedResult = ProductNewsFeedLayoutProps;

export const ProductNewsFeedTemplate = (props: ProductNewsFeedLayoutProps) => {
  const [layoutProps, setLayoutProps] = React.useState<ProductNewsFeedLayoutProps>(props);
  const [isLoading, setIsLoading] = React.useState(true);
  const [isRefreshing, setIsRefreshing] = React.useState(false);
  const scrollPaneRef = React.useRef<HTMLDivElement | null>(null);
  const itemRefs = React.useRef<Array<HTMLDivElement | null>>([]);

  const getPublishedTimestamp = React.useCallback((item: ProductNewsFeedItem): number | null => {
  ...
  const tocGroups = React.useMemo(() => {
    const groups: Array<{ label: string; entries: Array<{ item: ProductNewsFeedItem; index: number }> }> = [];
      ...
      <div className="mx-auto flex h-full w-full max-w-6xl flex-col gap-6">
        {isLoading ? (
          <section className="rounded-xl border border-dashed border-[hsl(var(--pnf-border))] bg-[hsl(var(--card))] px-5 py-12 text-center shadow-[inset_0_0_0_1px_hsl(var(--pnf-shadow-soft))]">
            <p className="text-sm text-[hsl(var(--muted-foreground))]">Loading feed...</p>
          </section>
        ) : (
          <>
            ...
            <section className="grid min-h-0 flex-1 grid-cols-1 gap-4 lg:grid-cols-[300px_minmax(0,1fr)]">
                <aside className="flex min-h-0 flex-col gap-4 rounded-xl border border-[hsl(var(--pnf-border))] bg-[hsl(var(--card))] p-4">
                  <div className="space-y-2 pb-3">
                    <p className="text-[11px] font-semibold uppercase tracking-[0.14em] text-[hsl(var(--pnf-accent))]">Product News</p>
                  ...
                  <h2 className="font-[var(--pnf-heading-font)] text-lg text-[hsl(var(--foreground))]">Contents</h2>
                      ...
                      tocGroups.map((group) => (
                        <div key={group.label} className="space-y-2">
                          ...
                          {group.entries.map(({ item, index }) => (
                            ...
                            >
                              <span className="block text-sm font-semibold text-[hsl(var(--foreground))]">{item.heading || 'Untitled update'}</span>
                              {item.publishedAtUtc ? (
                                <span className="mt-1 block text-xs text-[hsl(var(--muted-foreground))]">
                                  {new Date(item.publishedAtUtc).toLocaleDateString(undefined, { dateStyle: 'medium' })}
                                </span>
                              ) : null}
                            </button>
                          ))}
                  ...
                  <div className="mt-auto space-y-3 border-t border-[hsl(var(--pnf-border))] pt-3">
                    ...
                    >
                      {isLoading || isRefreshing ? 'Refreshing...' : 'Refresh feed'}
                    </button>

                    <p className="mt-2 text-xs text-[hsl(var(--muted-foreground))]">
                      Last updated:{' '}
                      {layoutProps.lastRetrievedUtc
                        ? new Date(layoutProps.lastRetrievedUtc).toLocaleString(undefined, {
                            dateStyle: 'medium',
                            timeStyle: 'short',
                          })
                        : 'Not available'}
                ...
                <section className="flex min-h-0 flex-col rounded-xl border border-[hsl(var(--pnf-border))] bg-[hsl(var(--pnf-surface))] p-3 md:p-4">
                  <div ref={scrollPaneRef} className="h-full space-y-5 overflow-y-auto pr-1 md:pr-2">
                    {tocGroups.length > 0 ? (
                      tocGroups.map((group) => (
                      ...
                      <section className="rounded-xl border border-dashed border-[hsl(var(--pnf-border))] bg-[hsl(var(--card))] px-5 py-12 text-center shadow-[inset_0_0_0_1px_hsl(var(--pnf-shadow-soft))]">
                        <h2 className="font-[var(--pnf-heading-font)] text-2xl text-[hsl(var(--foreground))]">No feed items yet</h2>
                        <p className="mt-2 text-sm text-[hsl(var(--muted-foreground))]">
                          Trigger a refresh to try loading new content from the community feed.
                        </p>
                      </section>
                    )}
                  </div>
...
export default ProductNewsFeedTemplate;
